...
bool StaticIMUInit::TryInit() {
    ...
    if (cov_gyro_.norm() > options_.max_static_gyro_var) {
        ROS_INFO( "[imu init] 陀螺仪测量噪声太大: %.3f > %.3f", cov_gyro_.norm(), options_.max_static_gyro_var);
        return false;
    }

    if (cov_acce_.norm() > options_.max_static_acce_var) {
        ROS_INFO( "[imu init] 加计测量噪声太大: %.3f > %.3f", cov_acce_.norm(), options_.max_static_acce_var);
        return false;
    }
